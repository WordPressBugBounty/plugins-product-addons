...
class WowRevenuePromotion {
	...
	private const VERSION              = '1'; // Cache buster.
	...
	public function __construct() {
		add_filter( 'revx_promo_promotion_hooks', array( $this, 'load' ) );
	...
	public function run_promotions() {
		...
		if ( $GLOBALS['revx_promo_promotion']['init'] ?? false ) {
			return;
		}
	...
	public function load( $callbacks ) {
		...
		$callbacks[ self::VERSION ] = function () {
			// Dismiss actions.
			add_action( 'wp_ajax_revx_promo_dismiss_promotion', array( $this, 'ajax_dismiss_promotion' ) );
			add_action( 'wp_ajax_revx_promo_install_promotion_plugin', array( $this, 'ajax_install_promotion_plugin' ) );

			// Promotions.
			// ------------------.

			// Add Options Tab.
			add_filter( 'woocommerce_product_data_tabs', array( $this, 'register_product_bundle_tab' ) );
			add_action( 'woocommerce_product_data_panels', array( $this, 'render_product_bundle_panel' ) );

			// Product category page.
			add_action( 'product_cat_add_form_fields', array( $this, 'render_product_category_add_notice' ) );

			// Linked product section notice.
			add_action( 'woocommerce_product_options_related', array( $this, 'render_linked_products_notice' ) );

			// WooCommerce settings header notice.
			add_action( 'admin_notices', array( $this, 'render_settings_header_notice' ) );

			// Marketing > Coupons notice.
			add_action( 'admin_notices', array( $this, 'render_coupons_page_notice' ) );
	...
	public function render_product_category_add_notice() {
		...
		$this->render_promotion_notice(
			'revx-product-category',
			'product_category_options',
			'Turn Categories into Bundle Offers',
			'display:none;',
			true,
			array(
				'default' => 'Start Now',
		...
		<script>
			jQuery( function( $ ) {
				$(document).ready(function() {
					$( '#revx-product-category' ).insertAfter( '#addtag').slideDown(300);
				});
			} );
		</script>
		<?php
	 ...
	 * @return array
	 */
	public function register_product_bundle_tab( $tabs ) {
		if ( ! is_array( $tabs ) || ! $this->should_show_promotion( 'upsell_and_bundle' ) ) {
			return $tabs;
		}

		$tabs['revx-promo-upsell-and-bundle'] = array(
			'label'    => 'Upsell & Bundle',
	 ...
	 * @return void
	 */
	public function render_product_bundle_panel() {
		if ( ! $this->should_show_promotion( 'upsell_and_bundle' ) ) {
			return;
		}

		?>
		<div id="revx-promo-add-product-options" class="panel woocommerce_options_panel">
			<?php
			$this->render_promotion_notice(
				'revx-promo-add-product-options',
				'upsell_and_bundle',
				'
					<div style="font-weight:bold;margin-bottom:12px;font-size:large;">
						Create High-Converting Bundle Offers in Minutes
					</div>
					<div style="margin-bottom:12px;">
						Bundle products, offer discounts, and increase your average order value - no setup hassle.
					</div>
					<ul style="padding-left: 1.5rem;">
						<li>
							...
							Increase order value with smart, strategic bundles
						</li>
						<li>
							<span style="font-weight:bold;">
								&#10003; &nbsp;
							</span>
							Turn browsers into buyers with irresistible discounts
						</li>
						<li>
							<span style="font-weight:bold;">
								&#10003; &nbsp;
							</span>
							Seamlessly integrates with your WooCommerce store
						</li>
					</ul>
				',
				'margin-inline: 20px;',
	...
	/**
	 * Render promotion notice in the linked products section.
	 *
	 * @return void
	 */
	public function render_linked_products_notice() {
		if ( ! $this->should_show_promotion( 'linked_products_section' ) ) {
			return;
		}
	...
	/**
	 * Render a header notice on the WooCommerce settings page.
	 *
	 * @return void
	 */
	public function render_settings_header_notice() {
		global $pagenow;

		$page = sanitize_text_field( wp_unslash( $_GET['page'] ?? '' ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended

		if (
			! $this->should_show_promotion( 'settings_header' ) ||
			'admin.php' !== $pagenow ||
			'wc-settings' !== $page
		) {
			return;
	...
	/**
	 * Render a notice below the coupons table on the Marketing > Coupons page.
	 *
	 * @return void
	 */
	public function render_coupons_page_notice() {
		global $typenow, $pagenow;

		if (
			'edit.php' !== $pagenow ||
			'shop_coupon' !== $typenow ||
			! $this->should_show_promotion( 'coupons_page' )
		) {
			return;
		}

		?>
		<div id="revx-promo-coupons-page-container" style="display:none;">
			<?php
			$this->render_promotion_notice(
		...
		<script>
			jQuery( function( $ ) {
				$(document).ready(function() {
					const $container = $( '#revx-promo-coupons-page-container' );
					const $tableNav = $( '#posts-filter' ).first();
					if ( $tableNav.length && $container.length ) {
						$container.insertAfter( $tableNav ).slideDown(300);
					}
				})
			} );
		</script>
	...
	public function render_products_submenu_modal() {
		...
		$nonce        = wp_create_nonce( 'revx_promo_promotion_nonce' );
		$img_src      = PRAD_URL . '/assets/img/wowrevenue-modal.png'; // CHANGE THIS.
		?>
		<style>
			...
			.revx-promo-products-modal.is-active {
				display: flex;
			}

			body.revx-promo-products-modal-open {
				overflow: hidden;
			}

			.revx-promo-products-modal *,
			.revx-promo-products-modal *::before,
			.revx-promo-products-modal *::after {
			...
			.revx-promo-products-modal__close {
				position: absolute;
				top: 18px;
				right: 18px;
				border: 0;
			...
			.revx-promo-products-modal__close:hover,
			.revx-promo-products-modal__close:focus {
				color: #0f172a;
				outline: none;
			}
			...
			.revx-promo-products-modal__list li:last-child {
				border-bottom: 0;
			}

			.revx-promo-products-modal__check {
				display: inline-flex;
				align-items: center;
				justify-content: center;
				width: 20px;
			...
			.revx-promo-products-modal__button:hover,
			.revx-promo-products-modal__button:focus {
				background: linear-gradient(180deg, #2563eb 0%, #1d4ed8 100%);
				color: #fff;
				outline: none;
			}

			.revx-promo-products-modal__button[aria-disabled="true"] {
				opacity: 0.75;
				cursor: wait;
			}

			.revx-promo-products-modal__learn {
			...
			.revx-promo-products-modal__meta {
				display: flex;
				align-items: center;
				justify-content: center;
				gap: 6px;
			...
			.revx-promo-products-modal__meta .dashicons {
				width: 16px;
				height: 16px;
				font-size: 16px;
			}
		...
		<div id="revx-promo-products-modal" class="revx-promo-products-modal" aria-hidden="true">
			<div class="revx-promo-products-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="revx-promo-products-modal-title">
				<button type="button" class="revx-promo-products-modal__close" aria-label="Close">&times;</button>
				<img class="revx-promo-products-modal__img"  src="<?php echo esc_attr( $img_src ); ?>" />
				<h2 id="revx-promo-products-modal-title" class="revx-promo-products-modal__title">
					Create High-Converting Bundle Offers in Minutes
				</h2>
				<p class="revx-promo-products-modal__text">
					Bundle products, offer discounts, and increase your average order value - no setup hassle.
				</p>
				<ul class="revx-promo-products-modal__list">
					<li><span class="revx-promo-products-modal__check">&#10003;</span><span>
						Increase order value with smart, strategic bundles
					</span></li>
					<li><span class="revx-promo-products-modal__check">&#10003;</span><span>
						Turn browsers into buyers with irresistible discounts
		...
		<script>
			jQuery( function( $ ) {
				const modal = $( '#revx-promo-products-modal' );
				const dialog = modal.find( '.revx-promo-products-modal__dialog' );
				const button = modal.find( '.revx-promo-products-modal__button' );
				...
				function openModal() {
					...
					$( 'body' ).addClass( 'revx-promo-products-modal-open' );
				}

				function closeModal() {
					modal.removeClass( 'is-active' ).attr( 'aria-hidden', 'true' );
					$( 'body' ).removeClass( 'revx-promo-products-modal-open' );
				...
				modal.find( '.revx-promo-products-modal__close' ).on( 'click', function() {
					closeModal();
				} );
				...
				button.on( 'click', function( event ) {
					...
					$.ajax( {
						...
						dataType: 'json',
						data: {
							action: 'revx_promo_install_promotion_plugin',
							nonce: nonce
						}
	...
	public function ajax_dismiss_promotion() {
		check_ajax_referer( 'revx_promo_promotion_nonce', 'nonce' );
	...
	private function get_dismiss_key( $type ) {
		return 'revx_promo_promotion_is_closed_' . self::VERSION . '_' . $type;
	}
	...
	private function should_show_promotion( $type ) {
		$ran_once = boolval( $GLOBALS['revx_promo_promotion'][ $type ] ?? false );
		if ( $ran_once ) {
			return false;
		}
	 ...
	 * @param string  $container_cls Additional inline styles for the inner container.
	 * @return void
	 */
			...
			jQuery( function( $ ) {
				$( document ).on( 'click', '<?php echo esc_js( $id ); ?> .notice-dismiss', function() {
					...
					$.ajax( {
						...
						type: 'POST',
						data: {
							action: 'revx_promo_dismiss_promotion',
							nonce: <?php echo wp_json_encode( wp_create_nonce( 'revx_promo_promotion_nonce' ) ); ?>,
	...
	private function get_dashboard_url() {
		return admin_url( 'admin.php?page=revenue#/' );
	}
