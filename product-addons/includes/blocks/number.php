...
<div 
	class="prad-parent prad-block-number prad-block-<?php echo esc_attr( $blockid . ' ' . $class ); ?>"
...
>
	...
	<div class="prad-d-flex prad-item-center prad-gap-12 prad-mb-12">
		<input 
			...
			id="<?php echo esc_attr( $blockid ) . '-prad-number-field'; ?>"
			min="<?php echo esc_attr( $min ? $min : 0 ); ?>"
			max="<?php echo esc_attr( $max ? $max : 0 ); ?>"
			step="<?php echo esc_attr( $step ? $step : 0 ); ?>"
			data-val="<?php echo esc_attr( $price_obj['price'] ); ?>"
		/>
		<?php if ( $pricePosition !== 'with_title' && $item->type != 'no_cost' ) : ?>
			<div class="prad-block-price prad-text-upper">
