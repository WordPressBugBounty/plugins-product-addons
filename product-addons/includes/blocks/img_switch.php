...
<div
	class="prad-parent prad-block-img-swatches prad-type_swatches-input prad-switcher-count prad-switcher-count-<?php echo esc_attr( $input_type ); ?> prad-block-<?php echo esc_attr( $blockid . ' ' . $class ); ?> prad-swatch-layout<?php echo esc_attr( $layout ); ?>"
...
>
	...
	<div class="prad-swatch-wrapper">
		...
		foreach ( $_options as $index => $item ) :
			...
			$thumbnail      = $updateProductImage && isset( $item->imgid ) && wp_get_attachment_image_src( $item->imgid, 'thumbnail' ) ? wp_get_attachment_image_src( $item->imgid, 'thumbnail' )[0] : '';
			$thumbnail_prop = $updateProductImage && isset( $item->imgid ) ? wc_get_product_attachment_props( $item->imgid ) : '';
			?>
			<div 
				class="prad-swatch-item-wrapper prad-relative prad-d-flex prad-flex-column prad-h-full"
				data-product-image="<?php echo esc_attr( wp_json_encode( $thumbnail ) ); ?>"
				data-product-image-prop="<?php echo esc_attr( wp_json_encode( $thumbnail_prop ) ); ?>"
				data-product-imageid="<?php echo esc_attr( isset( $item->imgid ) ? $item->imgid : '' ); ?>"
			>
				<div class="prad-swatch-container prad-p-2 prad-w-fit prad-relative prad-hover-<?php echo esc_attr( $hover_class ); ?>-bottom">
					...
					<label class="prad-lh-0 prad-mb-0" for="<?php echo esc_attr( $blockid . $index ); ?>">
						<img class="prad-swatch-item" title="<?php echo esc_attr( $price_obj['price'] ); ?>" src="<?php echo esc_url( ! empty( $img_url ) ? $img_url : PRAD_URL . 'assets/img/default-product.svg' ); ?>" alt="swatch item" />
					</label>
